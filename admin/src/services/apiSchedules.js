...
export const fetchAllRooms = async (token) => {
  const response = await api.get(`/api/v1/room/getAllRoom`, {
    headers: {
      Authorization: `Bearer ${token}`
    }
  });
...
export const confirmScheduleNow = async (scheduleId) => {
  const response = await api.patch(`/api/v1/schedule/confirmSchedule/${scheduleId}`);
  return response.data;
};
...
export const infoSchedules = async (token) =>{
 ...
 return {totalAdvogadosCadastrados, totalAgendamentos, totalMeetings, totalHearings}
